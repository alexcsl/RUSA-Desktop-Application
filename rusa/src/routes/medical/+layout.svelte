<!--
  Medical Department layout — shared sidebar for all /medical/* routes.
  Provides navigation contextual to the logged-in medical role.
...
<script lang="ts">
  ...
  function formatNotifText(n: NotificationItem): string {
    ...
    switch (n.type_) {
      case 'shift:allocated':
        return `New shift: ${p.shift_start ?? ''} → ${p.shift_end ?? ''}. Assigned by ${p.allocated_by ?? ''}`;
  ...
  const navLinks: NavLink[] = [
    // All medical staff
    { label: 'Patients', href: '/medical/patients', roles: ['all'] },
    { label: 'Log Treatment', href: '/medical/treatment-log', roles: ['all'] },
    { label: 'Inventory', href: '/medical/inventory', roles: ['all'] },
    { label: 'My Schedule', href: '/medical/schedule', roles: ['all'] },
    // HeadOfMedicine only
    { label: 'Staff & Shifts', href: '/medical/shifts', roles: ['HeadOfMedicine'] },
    { label: 'Messages', href: '/messaging/channels/medical-heads', roles: ['HeadOfMedicine'] },
    { label: 'Budget Request', href: '/medical/budget-request', roles: ['HeadOfMedicine'] },
    { label: 'Expenditure Report', href: '/medical/expenditure-report', roles: ['HeadOfMedicine'] },
    // Cross-role
    { label: 'Security Report', href: '/medical/security-report', roles: ['all'] },
  ...
  <header class="top-bar">
    ...
    <div class="user-section">
      ...
      {#if user}
        ...
        <span class="role-tag">{user.role === 'HeadOfMedicine' ? 'Head of Medicine' : 'Medical Staff'}</span>
...
<style>
  ...
  .role-tag { background:rgba(16,185,129,0.2);color:#34D399;padding:0.1rem 0.4rem;border-radius:4px;font-size:0.7rem; }
  ...
  .side-nav a.active { color:#10B981;background:rgba(16,185,129,0.1); }
