<!--
  Nomad: Assign Task — assign a task to a HeadOfSanitary or SettlerCommander.
  Access: TheNomad
-->
<script lang="ts">
  ...
  const NOMAD_TARGET_ROLES = ['SettlerCommander', 'HeadOfSanitary'];
  ...
  let targets = $derived(personnel.filter((p) => NOMAD_TARGET_ROLES.includes(p.role_name)));
...
<div class="page">
  <h2>Assign Task</h2>
  <p class="subtitle">Assign a task to a Settler Commander or Head of Sanitary.</p>

  {#if success}
    <div class="banner success">
      {success}
      <button class="link" onclick={() => goto('/directors/nomad/tasks')}>View all tasks →</button>
    </div>
  {/if}
  {#if error}
  ...
  {:else}
    <form onsubmit={handleSubmit} class="form">
      <div class="field">
        ...
        {#if targets.length === 0}
          <span class="hint">No eligible personnel (SettlerCommander / HeadOfSanitary) found.</span>
        {/if}
      </div>

      <div class="field">
        <label for="title">Task Title</label>
        <input id="title" type="text" bind:value={title} placeholder="e.g. Complete quarterly sanitation audit" required />
      </div>
      ...
      <div class="actions">
        ...
        <button type="button" class="btn-secondary" onclick={() => goto('/directors/nomad/tasks')}>
          View Tasks
        </button>
      </div>
    </form>
