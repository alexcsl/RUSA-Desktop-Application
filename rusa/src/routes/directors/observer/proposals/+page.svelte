<!--
  /directors/observer/proposals — TheObserver: Experiment Proposal Queue
  View and approve/deny experiment proposals routed from Chemist & Biologist.
-->
<script lang="ts">
  ...
  async function loadQueue() {
    loading = true; error = '';
    try { proposals = await getExperimentProposalQueue(); }
    catch (e: any) { error = e?.toString() ?? 'Failed to load proposals.'; }
    finally { loading = false; }
  }

  function openDecision(p: ExperimentProposalSummary) { deciding = p; decision = 'approved'; reason = ''; }
...
<p class="hint">Review proposals from Chemists and Biologists routed to you for bypass approval.</p>
...
{:else}
  <div class="table-wrap">
    <table>
      <thead>
        <tr><th>Title</th><th>Type</th><th>Proposed By</th><th>Status</th><th>Submitted</th><th>Action</th></tr>
      </thead>
      <tbody>
        {#each proposals as p}
          <tr>
          ...
          {#if p.description}
            <tr class="desc-row"><td colspan="6"><span class="desc-label">Description:</span> {p.description}</td></tr>
          {/if}
        {/each}
      </tbody>
    </table>
