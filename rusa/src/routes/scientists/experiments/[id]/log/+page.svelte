<!--
  /scientists/experiments/[id]/log — UC-PH-02 / UC-CH-05 / UC-BIO-03: Daily Log Form
-->
<script lang="ts">
  import { page } from '$app/stores';
  import { logExperimentSession } from '$lib/stores/scientists';
...
<div class="form-card">
  ...
  <div class="form-group">
    ...
    <textarea id="data" bind:value={collectedDataRaw} rows="4" placeholder="JSON object with collected measurements"></textarea>
  </div>
  <div class="btn-row">
    <button class="btn-primary" onclick={handleSubmit} disabled={submitting || !logDate}>
      {submitting ? 'Saving…' : 'Save Log Entry'}
    </button>
    <button class="btn-secondary" onclick={() => goto(`/scientists/experiments/${experimentId}`)}>Back to Experiment</button>
  </div>
