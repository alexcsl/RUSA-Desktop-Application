<!-- /sanitary/wastewater/docs — UC-WC-01: Manage Wastewater Treatment Documentation -->
<script lang="ts">
  import { onMount } from 'svelte';
  import {
    sanCreateWastewaterDoc, sanUpdateWastewaterDoc, sanGetWastewaterDocs,
    type WastewaterDoc,
  } from '$lib/stores/sanitary';

  let docs: WastewaterDoc[] = $state([]);
  let selected: WastewaterDoc | null = $state(null);
  let error = $state('');
  let success = $state('');
  let mode: 'view' | 'add' | 'edit' = $state('view');

  // Form
  let fTreatmentType = $state('');
  let fSteps: string[] = $state(['']);
  ...
  async function refresh() {
    error = ''; success = '';
    try { docs = await sanGetWastewaterDocs(); } catch (e: unknown) { error = String(e); }
  }

  function startAdd() {
    fTreatmentType = ''; fSteps = ['']; fSafetyReqs = ''; fComplianceNotes = '';
    mode = 'add'; selected = null;
  }

  function startEdit() {
    if (!selected) return;
    fTreatmentType = selected.treatment_type;
    fSteps = selected.steps && selected.steps.length > 0 ? [...selected.steps] : [''];
    fSafetyReqs = selected.safety_requirements ?? '';
    fComplianceNotes = selected.compliance_notes ?? '';
    mode = 'edit';
  }

  function addStep() { fSteps = [...fSteps, '']; }
  function removeStep(i: number) { fSteps = fSteps.filter((_, idx) => idx !== i); }

  async function handleSave() {
    error = ''; success = '';
    try {
      if (mode === 'add') {
        if (!fTreatmentType.trim()) { error = 'Treatment type required.'; return; }
        const cleanSteps = fSteps.filter((s) => s.trim());
        await sanCreateWastewaterDoc({ treatment_type: fTreatmentType.trim(), steps: cleanSteps.length > 0 ? cleanSteps : [], safety_requirements: fSafetyReqs.trim() || undefined, compliance_notes: fComplianceNotes.trim() || undefined });
        success = 'Document created.';
      } else if (mode === 'edit' && selected) {
        const cleanSteps = fSteps.filter((s) => s.trim());
        await sanUpdateWastewaterDoc({
          doc_id: selected.id,
...
<h1 class="title">Wastewater Treatment Docs</h1>
<p class="subtitle">UC-WC-01 — Maintain wastewater treatment documentation with step-by-step procedures.</p>
  ...
  <div class="list-panel">
    ...
    {#each docs as d}
      <button class="card" class:card-selected={selected?.id === d.id} onclick={() => { selected = d; mode = 'view'; }}>
        <span class="card-title">{d.treatment_type}</span>
        <span class="card-type">{d.steps.length} steps</span>
      </button>
    {:else}
      <p class="empty">No wastewater docs.</p>
    {/each}
  </div>

  <div class="detail-panel">
    {#if mode === 'view' && selected}
      <h2 class="detail-title">{selected.treatment_type}</h2>
      ...
      {#if selected.safety_requirements}<div class="detail-row"><span class="label">Safety Reqs</span><span>{selected.safety_requirements}</span></div>{/if}
      {#if selected.compliance_notes}<div class="detail-row"><span class="label">Compliance</span><span>{selected.compliance_notes}</span></div>{/if}

      {#if selected.steps && selected.steps.length > 0}
        <h3 class="steps-title">Treatment Steps</h3>
        <ol class="steps-list">
    ...
    {:else if mode === 'add' || mode === 'edit'}
      ...
      <div class="form-group"><label for="tt">Treatment Type</label><input id="tt" class="input" bind:value={fTreatmentType} placeholder="e.g. Biological, Chemical…" /></div>

      <div class="steps-edit">
        <span class="label">Steps</span>
        {#each fSteps as step, i}
          <div class="step-row">
            <span class="step-num">{i + 1}.</span>
            <input class="input step-input" bind:value={fSteps[i]} placeholder="Describe step…" />
            <button class="btn-icon" onclick={() => removeStep(i)}>✕</button>
          </div>
        {/each}
        <button class="btn-secondary btn-sm" onclick={addStep}>+ Add Step</button>
      </div>

      <div class="form-group"><label for="wsafety">Safety Requirements</label><textarea id="wsafety" class="input" rows="2" bind:value={fSafetyReqs} placeholder="Optional…"></textarea></div>
      <div class="form-group"><label for="wcomp">Compliance Notes</label><textarea id="wcomp" class="input" rows="2" bind:value={fComplianceNotes} placeholder="Optional…"></textarea></div>
...
<style>
  ...
  .card-type { font-size:0.7rem;color:#3ABEFF; }
  ...
  .steps-title { font-size:0.82rem;color:#F59E0B;margin:0.7rem 0 0.3rem; }
  .steps-list { margin:0;padding-left:1.2rem;color:#CBD5E1;font-size:0.8rem; }
  ...
  .steps-edit { margin-top:0.5rem; }
  .step-row { display:flex;gap:0.3rem;align-items:center;margin-bottom:0.25rem; }
  .step-num { color:#94A3B8;font-size:0.75rem;min-width:20px; }
  .step-input { flex:1; }
