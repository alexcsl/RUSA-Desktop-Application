<!-- /sanitary/security-report — Submit security incident report (all sanitary staff) -->
<script lang="ts">
  import { sanSubmitSecurityReport, type SanSubmitSecurityReportPayload } from '$lib/stores/sanitary';

  const INCIDENT_TYPES = [
    'unauthorized_access',
    'hazardous_material_breach',
    'equipment_failure',
    'chemical_spill',
    'physical_threat',
    'data_breach',
  ...
  async function handleSubmit(e: Event) {
    ...
    successMsg = '';
    try {
      const payload: SanSubmitSecurityReportPayload = {
      ...
      await sanSubmitSecurityReport(payload);
...
<form class="report-form" onsubmit={handleSubmit}>
  ...
  <div class="field">
    ...
    <input id="location" type="text" bind:value={location}
      placeholder="e.g. Disposal Bay 3, Wastewater Plant" required />
  </div>
  ...
  <div class="field">
    ...
    <textarea id="recommended-action" bind:value={recommendedAction} rows="3"
      placeholder="Optional: suggest a course of action…"></textarea>
  </div>
...
<style>
  ...
  .field input, .field select, .field textarea {
    background:#0E1428;border:1px solid rgba(245,158,11,0.2);color:#E6EDF3;border-radius:6px;
  ...
  .btn-submit {
    align-self:flex-start;background:rgba(245,158,11,0.15);border:1px solid #F59E0B;
    color:#F59E0B;border-radius:6px;padding:0.55rem 1.5rem;cursor:pointer;
