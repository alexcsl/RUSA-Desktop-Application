<!-- /directors/taskmaster/mission-completions — UC-TM-03: Mission Completion Approvals -->
<script lang="ts">
  import { onMount } from 'svelte';
  import {
    getCompletionRequestsTaskmaster, taskmasterDecideCompletion, getEvidenceUrls,
  ...
  onMount(async () => {
    try {
      requests = await getCompletionRequestsTaskmaster();
    } catch (e: unknown) {
  ...
  async function handleDecision() {
    ...
    processing = true;
    try {
      await taskmasterDecideCompletion({
        request_id: selected.id,
        decision,
        note: note || undefined,
      });
      success = decision === 'approved'
        ? 'Mission approved! Counters incremented for all assigned astronauts.'
        : 'Mission rejected. Returned to active.';
      requests = await getCompletionRequestsTaskmaster();
      selected = null;
    } catch (e: unknown) {
      error = e instanceof Error ? e.message : String(e);
...
<h1 class="title">Taskmaster — Mission Completions</h1>
<p class="subtitle">Final approval for mission completion requests forwarded by the Wanderer. Approving increments all assigned astronaut counters.</p>
...
{:else}
  <div class="grid">
    <div class="list-panel">
      ...
      {:else}
        <p class="empty">No pending mission completions.</p>
      {/each}
    </div>
    ...
    <div class="detail-panel">
      {#if selected}
        ...
        {#if selected.wanderer_note}
          <div class="note-box wanderer">
            <strong>Wanderer Note:</strong> {selected.wanderer_note}
          </div>
        {/if}
        ...
        <div class="action-form">
          ...
          <textarea class="textarea" bind:value={note} placeholder="Note for the astronaut(s)…" rows="2"></textarea>
      ...
      {:else}
        <div class="empty-state"><p>Select a completion request to review.</p></div>
      {/if}
    </div>
  </div>
{/if}
...
<style>
  ...
  .note-box { padding:0.5rem;border-radius:6px;font-size:0.8rem;margin-top:0.75rem;color:#CBD5E1; }
  .note-box.wanderer { background:rgba(139,92,246,0.1);border-left:3px solid #8B5CF6; }
  ...
  .decision-btn { padding:0.4rem 1rem;border-radius:6px;font-size:0.8rem;font-weight:600;cursor:pointer;border:2px solid transparent;background:rgba(255,255,255,0.03);color:#94A3B8; }
  .decision-btn.approve.active { border-color:#10B981;color:#10B981;background:rgba(16,185,129,0.1); }
  .decision-btn.reject.active { border-color:#EF4444;color:#EF4444;background:rgba(239,68,68,0.1); }
  .decision-btn:hover { color:#E6EDF3; }
  ...
  .st-pending { background:rgba(139,92,246,0.15);color:#C084FC; }
