<!--
  Sanitary Department layout — shared sidebar for all /sanitary/* routes.
  Provides navigation contextual to the logged-in sanitary crew role.
...
<script lang="ts">
  ...
  function formatNotifType(type_: string): string {
    const map: Record<string, string> = {
      'shift:allocated': 'Shift Assigned',
      'task:assigned': 'Task Assigned',
      'transfer:decided': 'Transfer Decision',
      'transfer:requested': 'Transfer Requested',
      'inspection_report:received': 'Inspection Report',
      'budget:expenditure_report': 'Expenditure Report',
      'vote:new': 'New Vote',
  ...
  function formatNotifText(n: NotificationItem): string {
    ...
    switch (n.type_) {
      case 'shift:allocated':
        return `New shift: ${p.shift_start ?? ''} → ${p.shift_end ?? ''} (${p.quarter ?? ''}). Assigned by ${p.allocated_by ?? ''}`;
      case 'task:assigned':
        return `${p.task_type ?? 'Task'}: ${p.task_name ?? 'Unnamed'} — Urgency: ${p.urgency ?? 'N/A'}. From ${p.assigned_by ?? ''}`;
      case 'transfer:decided':
        return `Your transfer was ${p.decision ?? 'decided'} by ${p.decided_by ?? ''}`;
      case 'transfer:requested':
        return `${p.requester ?? 'Staff'} requested a division transfer${p.reason ? `: ${p.reason}` : ''}`;
      case 'inspection_report:received':
        return `[${p.severity ?? '?'}] ${p.location ?? ''} — from ${p.reported_by ?? ''}`;
  ...
  function roleLabel(role: string | undefined): string {
    const labels: Record<string, string> = {
      HeadOfSanitary: 'Head of Sanitary',
      InspectorCrew: 'Inspector Crew',
      DisposalCrew: 'Disposal Crew',
      WastewaterCrew: 'Wastewater Crew',
  ...
  const navLinks: NavLink[] = [
    // Head of Sanitary
    { label: 'Budget Request', href: '/sanitary/head/budget/request', roles: ['HeadOfSanitary'] },
    { label: 'Expenditure Report', href: '/sanitary/head/budget/report', roles: ['HeadOfSanitary'] },
    { label: 'Inventory Mgmt', href: '/sanitary/head/inventory', roles: ['HeadOfSanitary'] },
    { label: 'Quarterly Scheduling', href: '/sanitary/head/scheduling', roles: ['HeadOfSanitary'] },
    { label: 'Assign Task', href: '/sanitary/head/tasks/new', roles: ['HeadOfSanitary'] },
    { label: 'All Tasks', href: '/sanitary/head/tasks', roles: ['HeadOfSanitary'] },
    { label: 'Transfer Requests', href: '/sanitary/head/transfers', roles: ['HeadOfSanitary'] },
    { label: 'Division Quotas', href: '/sanitary/head/quotas', roles: ['HeadOfSanitary'] },
    { label: 'Assign Recruit', href: '/sanitary/head/recruits', roles: ['HeadOfSanitary'] },
    // All Sanitary Staff (including Head)
    { label: 'My Tasks', href: '/sanitary/tasks', roles: ['all'] },
    { label: 'My Schedule', href: '/sanitary/schedule', roles: ['all'] },
    { label: 'Transfer Request', href: '/sanitary/transfer/request', roles: ['InspectorCrew', 'DisposalCrew', 'WastewaterCrew', 'CleanupCrew', 'TransportCrew'] },
    { label: 'Inventory Log', href: '/sanitary/inventory/log', roles: ['all'] },
    { label: 'Divisions', href: '/sanitary/divisions', roles: ['all'] },
    { label: 'Inspection Reports', href: '/sanitary/inspector/report', roles: ['all'] },
    // Disposal Crew
    { label: 'Disposal Docs', href: '/sanitary/disposal/docs', roles: ['DisposalCrew', 'HeadOfSanitary'] },
    // Wastewater Crew
    { label: 'Wastewater Docs', href: '/sanitary/wastewater/docs', roles: ['WastewaterCrew', 'HeadOfSanitary'] },
    // Shared for all roles
    { label: 'Security Report', href: '/sanitary/security-report', roles: ['all'] },
...
<div class="shell">
  ...
  {#if emergencyAlert}
    <div class="emergency-overlay" onclick={dismissEmergency} onkeydown={(e) => e.key === 'Escape' && dismissEmergency()} role="button" tabindex="-1">
      <div class="emergency-modal" onclick={(e) => e.stopPropagation()} onkeydown={() => {}} role="dialog" tabindex="-1">
...
<style>
  ...
  .role-tag { background:rgba(245,158,11,0.2);color:#F59E0B;padding:0.1rem 0.4rem;border-radius:4px;font-size:0.7rem; }
  ...
  .notif-type { color:#10B981;font-weight:600;min-width:100px;white-space:nowrap; }
  ...
  .side-nav a:hover { color:#E6EDF3;background:rgba(245,158,11,0.05); }
  .side-nav a.active { color:#F59E0B;background:rgba(245,158,11,0.1); }
