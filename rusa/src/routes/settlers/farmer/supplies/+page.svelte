<!--
  UC-FA-01: Request Farming Supplies (Farmer)
-->
<script lang="ts">
  import { stlRequestFarmingSupplies, type SupplyItem } from '$lib/stores/settlers';

  let supplies: SupplyItem[] = $state([{ item: '', specification: '', quantity: 1, reason: '' }]);
  let justification = $state('');
  let error = $state('');
  let success = $state('');
  let submitting = $state(false);

  function addRow() { supplies = [...supplies, { item: '', specification: '', quantity: 1, reason: '' }]; }
  function removeRow(i: number) { supplies = supplies.filter((_, idx) => idx !== i); }

  async function handleSubmit() {
    error = ''; success = '';
    const valid = supplies.filter(r => r.item.trim());
    if (valid.length === 0) { error = 'Add at least one supply item.'; return; }
    if (!justification.trim()) { error = 'Justification is required.'; return; }
    submitting = true;
    try {
      const id = await stlRequestFarmingSupplies({ items: valid, justification });
      success = `Farming supply request submitted (ID: ${id.slice(0,8)}…). Commander notified.`;
      supplies = [{ item: '', specification: '', quantity: 1, reason: '' }];
      justification = '';
...
<h2>Request Farming Supplies</h2>

<form class="form" onsubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
  <table class="tbl">
    <thead>
      <tr><th>Supply Item</th><th>Spec / Variety</th><th>Qty</th><th>Reason</th><th></th></tr>
    </thead>
    <tbody>
      {#each supplies as row, i}
        <tr>
          <td><input type="text" bind:value={row.item} placeholder="Seeds, fertiliser…" /></td>
          <td><input type="text" bind:value={row.specification} placeholder="Type / grade" /></td>
  ...
  <label>
    Justification *
    <textarea bind:value={justification} rows="3" required
      placeholder="Why are these farming supplies needed?"></textarea>
  </label>
