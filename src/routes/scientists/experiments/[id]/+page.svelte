<!--
  /scientists/experiments/[id] — Experiment detail + daily log list
-->
<script lang="ts">
  import { onMount } from 'svelte';
  import { page } from '$app/stores';
  import {
    getExperimentDetail, getFinalDocuments,
    type ExperimentSummary, type ExperimentLogSummary, type FinalDocSummary,
  } from '$lib/stores/scientists';
  ...
  let finalDocs: FinalDocSummary[] = $state([]);
  ...
  onMount(async () => {
    try {
      ...
      if (exp.status === 'closed') {
        try { finalDocs = await getFinalDocuments(experimentId); } catch {}
      }
    } catch (e: any) {
...
{:else}
  ...
  <div class="action-bar">
    {#if experiment.status === 'approved' || experiment.status === 'active'}
      <a href="/scientists/experiments/{experimentId}/log" class="btn-link">+ Log Session</a>
    {/if}
    {#if experiment.status === 'active'}
      <a href="/scientists/experiments/{experimentId}/conclusion" class="btn-link">Request Conclusion</a>
    {/if}
    {#if experiment.status === 'closed'}
      <a href="/scientists/experiments/{experimentId}/final-doc" class="btn-link">Submit Final Document</a>
    {/if}
  </div>
  ...
  {#if finalDocs.length > 0}
    <h3 class="section-title">Final Documents</h3>
    {#each finalDocs as d}
      <div class="doc-entry">
        <span class="tag">{d.doc_type}</span>
        <span>Status: <strong>{d.status}</strong></span>
        <span class="muted">Submitted: {new Date(d.created_at).toLocaleDateString()}</span>
      </div>
    {/each}
  {/if}
{/if}
...
<style>
  ...
  .doc-entry { display:flex;gap:0.75rem;align-items:center;padding:0.4rem 0;font-size:0.8rem;border-bottom:1px solid rgba(255,255,255,0.04); }
</style>
